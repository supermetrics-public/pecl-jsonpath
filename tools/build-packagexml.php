...
$release->notes = (function (string $name, string $version): string {
    if (function_exists('posix_isatty') && posix_isatty(STDIN)) {
        fwrite(STDOUT, "Open the release notes for $name $version at https://github.com/supermetrics-public/pecl-jsonpath/releases for editing\nand copy-paste the release notes in Markdown format (end with Ctrl-D):\n");
    }
    ...
    return cleanJsonPathReleaseNotes(mb_convert_encoding($notes, 'UTF-8'));
...
pecl install jsonpath-{$release->version}.tgz
Tag and publish version {$release->version} at https://github.com/supermetrics-public/pecl-jsonpath/releases
Upload the new release tgz to PECL at https://pecl.php.net/release-upload.php
rm jsonpath-{$release->version}.tgz
git checkout -b release/{$release->version}
git add php_jsonpath.h package.xml
git commit -m \"Publish version {$release->version}\"
...
<package version="2.0" xmlns="http://pear.php.net/dtd/package-2.0" xmlns:tasks="http://pear.php.net/dtd/tasks-1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://pear.php.net/dtd/tasks-1.0 http://pear.php.net/dtd/tasks-1.0.xsd http://pear.php.net/dtd/package-2.0 http://pear.php.net/dtd/package-2.0.xsd">
 ...
 <summary>Extract data using JSONPath notation</summary>
 <description>This extension allows you to use the versatile JSONPath query language to extract the properties that you want from a decoded JSON document.</description>
 <lead>
  <name>Viktor Djupsjöbacka</name>
  <user>viktor</user>
  <email>viktor.djupsjobacka@supermetrics.com</email>
  <active>yes</active>
 </lead>
 <lead>
...
function cleanJsonPathReleaseNotes(string $notes): string
{
    $notes = remove_emoji($notes);

    // Remove double whitespace from section headings (present due to the emojis)
    $notes = preg_replace('~^##  ([^\n]+)$~im', '## $1', $notes);

    // Remove empty line between section headings and their lists
    $notes = preg_replace('~^## ([^\n]+)$\n\n\-~im', "## \$1\n-", $notes);
